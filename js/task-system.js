(function (global) {
  ...
  function parseStudentList(value) {
    ...
    lines.forEach(line => {
      ...
      // Preferred formats: 3A,Lina / 3A，Lina / 3A<TAB>Lina.
      // Also accept the convenient format: 3A Lina.
      const delimited = line.match(/^(.+?)[,，\t;；]+(.+)$/);
      if (delimited) {
        className = delimited[1].trim();
        studentName = delimited[2].trim();
      } else {
        // Without punctuation, the first non-space token is treated as the class,
        // and the rest of the line is treated as the full student name.
